...
Proposals cover what is inside an attachment: a note's pitch, start, length and velocity,
adding and removing notes, a plugin parameter's value, a clip (moved along the song or to
another lane, copied, taken out, or given its own copy of the pattern it shares) and an
insert's chain (an effect added, removed, reordered or bypassed, a send added or removed).
Anything outside the attachment is refused as `OUT_OF_SCOPE`; making patterns, adding and
removing channels and writing automation curves are not proposable. A kind is announced in
capabilities only once a comparison can include it, because a model will try what it is
told about and offering a change nobody can hear before taking it is worse than not
offering it — clips and the plugins on a bus are in the project tree, which is what made
them previewable and so offerable. Scope for clips comes from a region attachment and for
chain changes from an insert attachment; nothing attached means nothing may change,
however precisely a reply names a clip. A copy of a clip points at the same pattern, so
the two are one part played twice and editing either edits both; making one unique is the
opposite, and taking a clip out removes where music is played rather than the music. Two
rules belong to the mixer alone: an effect has to be one this machine actually has, since
a chain built around a plugin that is not installed has a hole nobody finds until they
press play, and a send that would feed a signal back into itself is refused — it does not
get quieter — by the same test the app's own routing menu uses. A request that contradicts
itself is refused rather than half-obeyed: a copy with no destination, a removal that also
says where to, a make-unique that also moves; a position that is not on the chain is
refused rather than clamped to an end nobody asked for, an effect that is not on that
insert is `NOT_FOUND`, and a verb the service does not know is refused the way an unknown
note verb is. Notes are fenced in by the pattern and channel an
...
one stretch. It also says `same_notes_both_halves`, for the changes nobody can hear:
making a clip unique breaks the sharing and plays exactly the same music, and a person who
hears no difference has every reason to think the comparison is broken. It is read off
what each half actually played rather than off the kind of change, so it is equally true
of a proposal that moved a note and moved it back or one whose every change fell outside
what it was allowed to touch. It is not a claim that the two sound the same — a chain
change plays identical notes and sounds different, which is why the field was renamed from
`same_music_both_halves`: the app can compare notes, not sound. And the file says
`heard: false` and why: making a file is not listening to
...
no scanned VST has been through it. Every row of `docs/acceptance-2026-09-12.ko.md` that is
still blank, including the seven-step "한 곡을 끝까지" flow that is B6's completion
condition. And a small local model is not reliable at musical
judgement — this one read "up one tone" as one semitone, and it does not always stay inside
the notes it was given either: `llama3.1:8b` sometimes names a note it was shown for
context, the app refuses that with `OUT_OF_SCOPE` and the music is untouched.
`test_ai_connection` records that under "observed, not judged" rather than counting it as a
pass or a failure, because what a model writes varies between runs and a check that graded
it would be reporting the weather; what is asserted is the app's half — whatever the model
writes, either it was inside the lines or it is refused with a reason, and the music is
where it was either way. The app's promises held either
way, which is the point; the musical quality of a suggestion is the model's and is not
claimed here. Two P1s are open: `docs/review-v5-2026-09-13.ko.md` found that re-setting an
existing send diverges between the preview and the apply, and that the feedback check does
not follow send paths. Neither has a place in the suites below, so the passes do not
refute them.
