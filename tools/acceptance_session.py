...
def record_sheet(report, folder):
    ...
    lines = [
        ...
        "| 채팅 중 Delete / Backspace | 같은 상태에서 눌러 본다. 글자가 지워져야 하고 "
        "클립이나 노트가 지워지면 안 된다 | 이 셋과는 경로가 다르다. Space·Enter·Home은 "
        "전역 키 매핑이라 앱이 막아야 하지만, Delete는 포커스된 컴포넌트의 키 처리라 "
        "텍스트 박스가 먼저 가져간다 - 구조상 안전하다는 뜻이고, 구조를 믿는 것과 "
        "확인한 것은 다르다 | |",
        "| 패널 □ × 복원 · 줌 · 스크롤 · 이동/trim | 네 패널을 접었다 펴고, 줌과 스크롤을 "
        "쓰고, 클립을 옮기고 끝을 끌어 본다 | 스크립트가 핸들러는 부를 수 있어도 창 "
        "관리와 스크롤 관성이 사람 손에 어떻게 느껴지는지는 못 본다 | |",
        ...
        "",
        "배율도 같이 본다. 기계는 잘리는 글자만 쟀다: 100%와 150%는 없고, 200%와 작은 창(700×480)에서는",
        "Channel Rack 아래 버튼 이름(`Piano roll`, `Remove channel`)이 잘린다. 읽기 어려운지는 재지 못했다.",
        "잘리는 곳과 읽기 어려운 곳을 적는다.",
        "",
        "| 배율 | 잘리거나 읽기 어려운 곳 |",
        "|---|---|",
        "| 100% | |",
