...
## 필수 패널 동작 — 2026-09-11 사용자 확정

기본 배치는 출발점이며 고정 비율로 강제하지 않는다. Browser, Channel Rack, Mixer, Pattern Picker, Playlist 각각의 크기를 경계 드래그로 조절하고, 패널 하나를 앱 작업 영역 전체로 최대화할 수 있어야 한다. Piano Roll도 편집 영역을 크게 쓸 수 있어야 한다.

- 패널 헤더의 최대화/복원 버튼, 헤더 더블클릭, View 메뉴 명령으로 같은 동작을 제공한다. 최대화 시 전역 transport와 메뉴는 유지하고 다른 패널과 분할선만 임시로 숨긴다.
- 사용자 지정 버튼 배치: 헤더 오른쪽에 Windows 관례의 `— □ ×` 순서로 둔다. `—`는 작업 영역 하단의 패널 탭으로 최소화하고 클릭하면 이전 배치로 복원한다. `□`는 작업 영역 최대화, 최대화 중에는 겹친 네모 복원 아이콘으로 바꾼다. `×`는 패널만 숨기며 View 메뉴에서 다시 연다. 어떤 버튼도 트랙·클립·노트를 삭제하거나 앱을 종료하지 않는다. 앱 바깥 Windows 타이틀바 버튼은 원래 OS 동작을 유지한다. 각 버튼에 최소화/최대화 또는 복원/패널 닫기 툴팁과 접근성 이름을 붙인다.
- 복원하면 이전 크기·보임/숨김·선택·스크롤·줌·포커스로 돌아간다. 최대화를 위해 다른 패널을 삭제하거나 다시 만들지 않는다. 숨겨진 패널의 모델 동기화는 계속된다.
- 창 자체의 Windows 최대화와 패널 최대화는 별개다. 최소 크기와 내부 스크롤을 제공하고, 작은 창/DPI 변경 후에도 복원 버튼이 화면 밖으로 나가지 않게 한다.
- 배치 저장·복원 및 기본 배치 초기화를 제공한다. 패널 조절은 음악 Undo 기록을 소비하지 않아야 한다. 레이아웃만 바꿨을 때 외부 음악 편집 revision이 불필요하게 충돌하는지도 확인한다.
- 완료 검사: 각 패널 resize→maximize→restore, 최대화 중 외부 편집/재생, 창 크기·DPI 변경, 저장 후 재실행. 원래 분할과 숨김 상태가 보존되어야 한다.

현재 코드 확인: Workspace.h에는 StretchableLayoutResizerBar 기반 크기 조절과 visible/sizes 저장이 있다. 패널 하나를 최대화하고 이전 배치를 한 번에 복원하는 명시적 기능은 확인되지 않았다. 기존 resize 구현을 재사용해 추가할 요구사항이며, 문서 반영만으로 구현 완료로 처리하지 않는다.
