...
세 번째가 실제 모델에 대한 검사다. 앞의 둘과 갈라 둔 이유가 있다. 앞의 둘은 echo bridge를 써서 답을 미리 알 수 있어야 배선을 검사할 수 있고, 그 방식으로는 연결을 검사할 수 없다. 이 검사는 **말에 대해 아무것도 단언하지 않는다.** 마일스톤이 지목한 세 질문(구간이 이상하다 / Insert의 플러그인을 봐 달라 / 이 노트들을 설명해 달라)과 후속 질문 하나를 던지고, 모델이 무엇을 쓰든 성립해야 하는 것만 확인한다. 답이 붙인 것에 대한 답인지, 받은 적 없는 오디오를 들었다고 하지 않는지, 후속 질문이 앞의 대화를 보는지, 기다리는 동안 재생이 계속되는지, 묻고 답하는 것이 revision을 하나도 올리지 않는지. 모델이 올라와 있지 않으면 통과가 아니라 종료 코드 2다.
