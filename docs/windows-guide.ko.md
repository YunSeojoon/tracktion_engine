...
| `limits` | 이 미리듣기에 들어가지 않은 것, 또는 렌더가 실패한 이유 |
...
제안이 **믹서 파라미터도 바꾸는 경우 미리듣기에는 노트만 들어간다.** 파라미터는 트리가 아니라 플러그인 안에 있고 미리듣기는 사본의 플러그인을 열지 않기 때문이다. 그때 `limits`에 그렇게 적히고 채팅에도 나온다. **절반을 조용히 빼놓은 비교본은 만들지 않는다.**
...
"메모"라고 불리는 것이 사실 셋이며, 앱은 그것을 **섞지 않는다.**
...
`condition`·`guess`·`request`는 두 번째 자리에 글을, `accept`와 `remove`는 메모 id를 받는다. `accept`는 짐작에만 통하고 조건을 하나 새로 만든다. 지금 있는 메모 전부는 `chat-inspector.json`의 `notes`에 id·종류·글·시각·읽은 revision·어느 짐작에서 왔는지까지 그대로 나온다.
...
- **A/B를 만드는 버튼도, 메모를 쓰는 편집기도 화면에 없다.** 둘 다 진단용 `--ui-script`(`{"preview": ...}`, `{"project_note": ...}`)로만 부른다.
- **만들어진 A/B를 듣고 고르는 것은 사람의 몫이다.** 앱은 두 파일을 만들고 `heard: false`라고 적는 데까지만 한다.
- **A/B를 파일 해시로 증명할 수 없다.** 이 기계의 렌더는 비트 단위로 재현되지 않으므로 같은 음악을 두 번 렌더해도 해시가 늘 같지는 않다. 확인할 수 있는 것은 엔진이 연주할 노트까지다.
- **메뉴가 포인터 아래에 뜨는지와 읽기 좋은지는 검사되지 않는다.** 검사는 우클릭이 메뉴 경로에 닿는다는 것과 그것이 곡을 바꾸지 않는다는 것까지만 본다.
- **미리듣기에 파라미터 변경은 들어가지 않는다.** 그런 제안은 노트 부분만 들리고, `preview-status.json`의 `limits`가 그렇다고 적는다.
...
| `notes.json` | 이 프로젝트의 메모. 조건·짐작이 들어가며 `project_id`에 묶인다. 요청은 저장되지만 다시 열 때 복원되지 않는다 |
| `preview-status.json` | 마지막 A/B의 결과. 두 파일의 지문, 나온 revision, 구간, 그리고 `heard: false` |
...
`test_preview.py`는 A/B가 **두 개의 진짜 파일과 바뀌지 않은 곡**인지를 본다. 두 파일이 만들어졌는지, 서로 다른 오디오인지, 둘 다 비어 있지 않은지, 어느 revision과 어느 구간에서 나왔다고 말하는지, 그리고 렌더가 끝난 뒤 **revision이 그대로이고 제안이 아직 적용되지 않은 제안이며 패턴의 노트가 하나도 움직이지 않았는지**를 확인한다. 그 뒤에 곡을 실제로 고쳐 놓고, 디스크에 남은 미리듣기가 **여전히 자기가 나온 revision을 말하는지** — 즉 오래된 비교본을 알아볼 수 있는지 — 를 본다. "제안이 실제로 더 좋게 들리는가"는 통과 목록에 없다. `FOR A PERSON`에 적힌다.

`test_project_notes.py`는 세 종류가 섞이지 않는지를 본다. 짐작이 조건으로 기록되지 않는 것, 승격하면 조건이 생기고 **짐작도 남는 것**, 조건과 짐작은 앱을 다시 열어도 돌아오고 **끝난 요청은 돌아오지 않는 것**, 폴더를 복사한 프로젝트가 **아무것도 물려받지 않는 것**, 그리고 프롬프트가 셋을 라벨로 구분해 적는 것이다. 메모를 쓰는 동안 revision이 움직이지 않는 것도 함께 본다.
...
2026-09-12 기준 최신 실행은 **한 바이너리에서, 순차로, 다른 것이 앱을 쥐고 있지 않은 상태로** 돌렸다. sha256 `347ba03c794f209f04ae8cbb8c1807df6cfc4ecd47d99ff20727eec32f5c70fd`이며 실행 전후의 해시가 같다. 도구 계약·제안 경계·프로젝트 메모·AI 채팅·채팅 신뢰성·AI 연결·DAW 조작·미리듣기 여덟 suite가 `FAILURES: none`(개별 ok 줄 261개)이고 `tools/test_live_sync.py`는 28개를 통과했다. 확인되지 **않은** 것은 위 [지금 할 수 없는 것](#지금-할-수-없는-것)과 같다 — A/B의 청음, 메뉴가 화면에 어떻게 보이는지, 텍스트 입력란이 진짜로 키보드를 쥔 상태의 키 동작, hosted OpenAI 경로.
